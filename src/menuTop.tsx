...
export function MenuTop(props: {
  ...
  const btnOrtho = <button onClick={() => props.setOrtho(!props.ortho)} className={"rounded-full text-white h-8 w-8 flex items-center justify-center transition-all hover:scale-110 hover:bg-hover-t40"} type="button">{props.ortho ? <Icons.Perspective height="20" width="20" fill="currentColor" /> : <Icons.Orthographic height="20" width="20" fill="currentColor" />}</button>
  const btnCamera = <button className={"rounded-full text-white text-sm h-8 w-8 flex items-center justify-center transition-all hover:scale-110 hover:bg-hover-t40 opacity-75"} type="button"><Icons.Camera height="20" width="20" fill="currentColor" />{speed}</button>
